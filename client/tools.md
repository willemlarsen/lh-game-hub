# The Tools Used in `app`

## Introduction

`app` is standards-based, so it uses all the usual tools to manage
...
project makes use and how they fit in to the `app` picture.

## Git

[Git](http://git-scm.com/) is a distributed version control system.
`app` uses git to manage its codebase. While in theory you don't have
to use Git once you download `app`, this project makes the assumption
...
While `app` makes heavy use of Node.js behind the scenes, you as the
...
you first start with `app` you have to tell NPM to install the
...
Don't worry about knowing Node.js in order to use `app`; Grunt is
...
to get started with `app`. But for those curious or looking to go a
...
Grunt is the engine behind `app`. It's the magic that makes it move.
...
`app` comes with a `bower.json` file that looks something like this:
...
in `app` by default in there to see what I mean.
